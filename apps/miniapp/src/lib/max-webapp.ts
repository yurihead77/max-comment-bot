...
export function isInitDataReadyForAuth(initData: string): boolean {
 ...
 */
export async function waitForInitData(maxWaitMs = 3000, stepMs = 120): Promise<string> {
  const startedAt = Date.now();
  while (Date.now() - startedAt < maxWaitMs) {
    const initData = getInitData();
    if (isInitDataReadyForAuth(initData)) return initData;
